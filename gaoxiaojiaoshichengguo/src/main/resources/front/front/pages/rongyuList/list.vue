<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="search-form round">
				<input v-model="searchForm.rongyuName" type="text" placeholder="标题"></input>
		</view>
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
            ...
            :style='{"borderRadius":"40rpx","backgroundColor":"#47EDB5"}'
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.rongyuPhoto?product.rongyuPhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('rongyuList','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('rongyuList','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('rongyuList','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let rongyuYesnoTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'rongyu_yesno_types'
		  	});
		  	this.rongyuYesnoTypesList = rongyuYesnoTypes.data.list;
		...
		methods: {
			...
			async upCallback(mescroll) {
				let params = {
					...
					limit: mescroll.size
                    ,rongyuDelete: 1,
					jiaoshiId: this.user.id,
				}


                let res = await this.$api.list(`rongyu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('rongyu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.rongyuName) {
                    searchForm['rongyuName'] = this.searchForm.rongyuName
                }
				if (this.searchForm.rongyuTypes) {
                    searchForm['rongyuTypes'] = this.searchForm.rongyuTypes
                }
				if (this.searchForm.rongyuName) {
					searchForm['rongyuName'] = this.searchForm.rongyuName
				}

                let res = await this.$api.list(`rongyu`, searchForm);
				// 如果是第一页数据置空
