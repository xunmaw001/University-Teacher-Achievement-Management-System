		...
		<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
			<view style="margin: 10rpx 10rpx 10rpx 10rpx;" @tap="onDetailTap(product)" class="listm flex flex-between"
				...
				<image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill"
					class="listmpic" :src="product.yonghuPhoto?product.yonghuPhoto.split(',')[0]:''"></image>
				<view class="listmr">
				...
				<view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
					<text style="margin-left: 50rpx;" v-if="isAuth('yonghuList','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">
						修改
					</text>
					<text v-if="isAuth('yonghuList','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除
					</text>
				</view>
			</view>
		</view> <button
			...
			v-if="isAuth('yonghuList','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			let yonghuYesnoTypes = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode: 'yonghu_yesno_types'
			});
			this.yonghuYesnoTypesList = yonghuYesnoTypes.data.list;
		...
		methods: {
			...
			async search() {
				...
				if (this.searchForm.chengguoName) {
					searchForm['chengguoName'] = this.searchForm.chengguoName
				}
